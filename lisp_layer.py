...
class AFIRecord(Packet):
    name = "AFI Record"
    fields_desc = [
        ShortField("afi", 2),
        ConditionalField(IPField("ipv4address", '192.168.1.1'), lambda pkt:pkt.afi==1),
        ConditionalField(IP6Field("ipv6address", '2001::1'), lambda pkt:pkt.afi==10)
    ] 

class BitPacketLenField(BitField):
    def __init__(self, name, default, size, length_of=None, count_of=None, adjust=lambda pkt,x:x):
        ...
        self.adjust=adjust
    def i2m(self, pkt, x):
        return BitField.i2m(self,pkt,FieldLenField.i2m.im_func(self, pkt, x))

class LISPRequest2(Packet):
    ...
    fields_desc = [
        ...
        BitField("reserved_fields", 0, 9),
        BitPacketLenField("itr_rloc_count", None, 5, count_of="itr_rloc_list",adjust=lambda pkt,x:x-1),
        BitField("recordcount", 0, 8),
        ByteField("nonce", 8),
        PacketField("src_eid", None, AFIRecord),
        PacketListField("itr_rloc_list", None, AFIRecord, count_from=lambda pkt:pkt.itr_rloc_count) 
    ]
...
bind_layers( UDP, LISPRequest2, dport=4342)
